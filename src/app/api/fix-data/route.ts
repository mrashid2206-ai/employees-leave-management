...
export async function POST(request: Request) {
  ...
  for (const rec of march31) {
    ...
    if (changed) {
      if (checkIn && checkOut) {
        ...
        results.push(`March31 fix: emp ${rec.employee_id} check_in ${rec.check_in}→${checkIn}, work=${workHours}h, OT=${overtime}h`)
      } else {
        await pool.query('UPDATE attendance SET check_in = $1 WHERE id = $2', [checkIn, rec.id])
        results.push(`March31 fix: emp ${rec.employee_id} check_in ${rec.check_in}→${checkIn}`)
      }
    }
  }

  // 2. Fix tardiness hours_late_decimal (recalculate from minutes_late / 60)
  ...
  results.push(`Tardiness hours fixed: ${tardinessFixed} records (now minutes/60 instead of minutes/1440)`)

  // 3. Recalculate leave days_count to exclude weekends and holidays
  ...
  for (const leave of allLeaves) {
    ...
    if (count !== leave.days_count) {
      ...
      results.push(`Leave #${leave.id}: days ${leave.days_count}→${count} (${leave.start_date} to ${leave.end_date})`)
      leavesFixed++
    }
  }
  results.push(`Leave days_count fixed: ${leavesFixed} records`)

  // 4. Recalculate all employee leave_balance from approved leaves
  ...
  for (const emp of employees) {
    ...
    if (current[0].leave_balance !== correctBalance) {
      ...
      results.push(`Balance fix: emp ${emp.id} balance ${current[0].leave_balance}→${correctBalance} (used: ${totalUsed})`)
      balancesFixed++
    }
  }
  results.push(`Balances recalculated: ${balancesFixed} employees`)
